...
cpdef ABRA_LL(double[::1] freq, double[:, ::1] PSD, int N_Days, double A, 
              double ma, double v0_Halo, double vDotMag_Halo, double alpha_Halo, double tBar_Halo,
              double v0_Stream, double vDotMag_Stream, double alpha_Stream, double tBar_Stream,
              double fracStream, double PSDback, int includeGF):
    ...
    cdef double vObs_Halo, vObs_Stream

    # Construct full LL
    cdef double ll = 0.
    with nogil:
        for iday in range(N_Days):
            vObs_Halo = sqrt(vDotMag_Halo**2 + vCrossMag**2 + \
                        2*vDotMag_Halo * vCrossMag *alpha_Halo*cos(omega*(iday - tBar_Halo)))
            vObs_Stream  = sqrt(vDotMag_Stream**2 + vCrossMag**2 + \
                           2*vDotMag_Stream * vCrossMag *alpha_Stream*cos(omega*(iday - tBar_Stream)))

            for ifrq in range(N_frq):
                LambdaK_val = LambdaK(freq[ifrq], ma, A, PSDback, v0_Halo, vObs_Halo, \
                              v0_Stream, vObs_Stream, fracStream, iday, includeGF)
...
cdef double v0 = 220.0 #velocity dispersion km/s
cdef double omega = 2. * np.pi / 365. # angular velocity rad/s
cdef double tp = -74.88 #0.0 # days
...
cdef double LambdaK(double freq, double ma, double A, double PSDback,
                    ...
                    double v0_Stream, double vObs_Stream,
                    double fracStream, double day, int includeGF) nogil:

    cdef double vSq = 2.*(2.*pi*freq-ma)/ ma

    if vSq > 0:
        return A * pi * (1.0-fracStream)*(fSHM(sqrt(vSq), v0_Halo/c, vObs_Halo/c) \
               + c*f1(c*sqrt(vSq),v0, day, includeGF)) / ma / sqrt(vSq) \
               + A * pi * fracStream * (fSHM(sqrt(vSq), v0_Stream/c, vObs_Stream/c) \
...
cdef double f1(double vMag, double v0, double t, int includeGF) nogil:
    ...
    cdef double theta, phi, vx, vy, vz, temp_norm, \
                temp_unit_x, temp_unit_y, temp_unit_z, temp
